	...
	function calculateCompatibilityScore(player: Player): number {
		...
		if (currentUser.experienceLevel && player.experienceLevel) {
			if (currentUser.experienceLevel === player.experienceLevel) {
				...
				(currentUser.experienceLevel === 'intermediate' && ['beginner', 'advanced'].includes(player.experienceLevel)) ||
				(player.experienceLevel === 'intermediate' && ['beginner', 'advanced'].includes(currentUser.experienceLevel))
			) {
		...
		{#each sortedPlayers as player}
			...
			<div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
						...
						<div class="mt-2 text-sm text-gray-600 space-y-1">
							...
							{#if player.vibePreference}
								<div class="flex items-center space-x-1">
									...
									<span class="capitalize">{player.vibePreference === 'both' ? 'Flexible' : player.vibePreference}</span>
								</div>
							{/if}
				...
				<div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
					<!-- Shared Availability -->
					<div>
						<h4 class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Shared Availability</h4>
					...
					<div>
						<h4 class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Preferred Games</h4>
						...
						{:else if player.gamePreferences.length > 0}
							<div class="flex flex-wrap gap-1">
								{#each player.gamePreferences.slice(0, 4) as game}
									...
									<span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {isShared ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700'}" title="Game ID: {game.gameBggId}, Shared: {isShared}">
										{game.name}
									</span>
								{/each}
